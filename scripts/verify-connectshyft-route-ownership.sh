...
ADMIN_SRC_DIR="$ROOT_DIR/apps/admin-web/src"
...
if rg -q "/app/connectshyft" "$ADMIN_SRC_DIR"; then
  echo "Route ownership check failed: admin-web still references /app/connectshyft routes"
  rg -n "/app/connectshyft" "$ADMIN_SRC_DIR" || true
  exit 1
fi
