...
const resolveJwtModule = () => {
  ...
  try {
    return require(path.join(root, 'apps/moneyshyft-api/node_modules/jsonwebtoken'));
  } catch (_error) {
    // Continue to explicit failure with remediation.
  }

  throw new Error(
    'Missing jsonwebtoken dependency. Install with `npm ci` or `cd apps/moneyshyft-api && npm ci` before running Epic 0 quality gates.'
  );
};
...
const backendEnv = parseDotEnv(path.join(root, 'apps/moneyshyft-api/.env'));
