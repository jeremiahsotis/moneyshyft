...
const usage = () => {
  console.log(`RouteShyft WP direct-write shutdown

Usage:
  node scripts/routeshyft/wp-cutover/shutdown-wp-direct-writes.mjs [options]
...
Required environment:
  ...
  WP_ROUTE_TABLES             (comma-separated RouteShyft authority tables in WP DB)
...
Examples:
  ...
  node scripts/routeshyft/wp-cutover/shutdown-wp-direct-writes.mjs

  # apply + full verification
  node scripts/routeshyft/wp-cutover/shutdown-wp-direct-writes.mjs --apply
`);
};
...
const runReconcileOnlyCheck = (artifactDir) => {
  ...
  const result = runCommand(
    'node',
    [
      'scripts/routeshyft/wp-cutover/rehearse-route-cutover.mjs',
      '--reconcile-only',
      '--output',
      reconcileReportPath,
      '--normalized-output',
