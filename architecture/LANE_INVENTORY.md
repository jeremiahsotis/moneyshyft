...
| apps/moneyshyft-api/src/modules/connectshyft            | module tree   | CONNECTSHYFT                  | deleted_after_slice_10c  | connectshyft-api                           | dead_stale           | safe_delete_after_convergence | deleted in Slice 10c after the stale top-level MoneyShyft identity tests were removed, the remaining async-only boundary assertion moved to canonical ConnectShyft coverage, and post-delete importer scans returned no live MoneyShyft references |
| apps/moneyshyft-api/src/modules/connectshyft/__tests__  | test mirror   | CONNECTSHYFT                  | deleted_after_slice_10c  | connectshyft-api canonical tests preferred | dead_stale           | safe_delete_after_convergence | deleted in Slice 10c together with the MoneyShyft ConnectShyft mirror tree after proof confirmed canonical ConnectShyft test ownership |
...
| apps/admin-api/src/modules/connectshyft                 | module tree   | CONNECTSHYFT support mirror   | deleted_after_slice_10c  | connectshyft-api                           | dead_stale           | safe_delete_after_convergence | deleted in Slice 10c after `routes/api/v1/platform-admin-console.ts` and its route test moved to the shared `libs/platform/src/connectshyftNumberMappings.ts` boundary via `src/platform/connectshyftNumberMappings.ts`, the top-level Admin identity tests were removed, and post-delete importer scans returned no live Admin references |
| apps/admin-api/src/modules/connectshyft/__tests__       | test mirror   | CONNECTSHYFT support mirror   | deleted_after_slice_10c  | connectshyft-api canonical tests preferred | dead_stale           | safe_delete_after_convergence | deleted in Slice 10c together with the Admin ConnectShyft mirror tree after the Admin console no longer depended on mirror `numberMappings` |
