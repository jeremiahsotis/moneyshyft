...
| apps/connectshyft-api/src/services/PlatformAdminService.ts | service mirror | ADMIN entitlement mirror | non_runtime_test_only | admin-api for full service behavior; `libs/platform/src/tenantModuleEntitlements.ts` for shared entitlement subset | transitional | safe_delete_after_convergence | Slice 9 removed the live `connectshyft.ts` anchor; remaining use is deferred test-only support |
