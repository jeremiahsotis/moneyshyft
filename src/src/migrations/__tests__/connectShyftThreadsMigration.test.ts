...
describe('20260224170000_create_connectshyft_threads migration', () => {
  it('creates canonical connectshyft thread schema and lifecycle constraints idempotently', async () => {
    ...
    expect(rawCalls.some((sql: string) => sql.includes('next_evaluation_at_utc TIMESTAMPTZ NOT NULL DEFAULT NOW()'))).toBe(true);
    ...
    expect(rawCalls.some((sql: string) => sql.includes("WHERE state <> 'CLOSED'"))).toBe(true);
