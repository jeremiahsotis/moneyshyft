...
- TypeScript (ES2022) on Node.js >=20 + Express, Knex, pg, Jest, Supertest, existing ConnectShyft runtime modules under `apps/moneyshyft-api/src/modules/connectshyft` (009-connectshyft-sms-target-resolution)
- Shared PostgreSQL, primarily `connectshyft.cs_threads`, `connectshyft.cs_neighbors`, `connectshyft.cs_neighbor_phones`, plus existing lifecycle/event metadata surfaces already used by the runtime (009-connectshyft-sms-target-resolution)
