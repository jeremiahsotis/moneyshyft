...
| `apps/connectshyft-api/src/services/*` money-domain service tree | unmounted stale mirror | `connectshyft-api` mounts only ConnectShyft runtime. Slice 9 removed the live `PlatformAdminService` anchor from `src/routes/api/v1/connectshyft.ts`; broader service-tree deletion still waits for Slice 10 proof. |
| `apps/moneyshyft-api/src/services/PlatformAdminService.ts` and `apps/connectshyft-api/src/services/PlatformAdminService.ts` | stale entitlement mirrors with split Slice 10 outcomes | Slice 9 rewired the live runtime anchors to `libs/platform/src/tenantModuleEntitlements.ts`. Slice 10 deleted the isolated ConnectShyft mirror and paired stale service test, while the MoneyShyft mirror remains deferred because route-level in-repo blockers still import it. |
