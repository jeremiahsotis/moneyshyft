...
| `apps/moneyshyft-api/src/services/PlatformAdminService.ts` and `apps/connectshyft-api/src/services/PlatformAdminService.ts` | stale entitlement mirrors with deferred residue | Slice 9 rewired the live runtime anchors to `libs/platform/src/tenantModuleEntitlements.ts`. Remaining references are unmounted or test-only and should be treated as deferred cleanup surfaces rather than live owners. |
