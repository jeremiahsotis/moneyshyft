...
const FIXTURE_SCHEMA = 'platform_mutation_contract_moneyshyft';
const TEST_TENANT_ID = '55555555-5555-4555-8555-555555555555';
const TEST_ACTOR_ID = '66666666-6666-4666-8666-666666666666';
const SUCCESS_EVENT_NAME = 'kernel.mutation.accepted.moneyshyft-contract';
const FAIL_EVENT_NAME = 'kernel.mutation.force-outbox-fail.moneyshyft-contract';
const OUTBOX_FAILURE_FUNCTION = 'raise_outbox_fail_moneyshyft_contract';
const OUTBOX_FAILURE_TRIGGER = 'trg_outbox_fail_moneyshyft_contract';
...
describeIfDb('executePlatformMutation (postgres contract)', () => {
  ...
  beforeAll(async () => {
    ...
    await db.withSchema('platform').table('tenants').insert({
      id: TEST_TENANT_ID,
      name: 'MoneyShyft Contract Tenant',
      status: 'active',
    }).onConflict('id').ignore();

    await db('users').insert({
      id: TEST_ACTOR_ID,
      email: 'moneyshyft-contract@example.com',
      password_hash: 'not-used',
      first_name: 'MoneyShyft',
      last_name: 'Contract',
      role: 'member',
    }).onConflict('id').ignore();
