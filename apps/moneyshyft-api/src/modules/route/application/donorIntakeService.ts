...
export type DonorIntakeRefusalResult = {
  ok: false;
  code:
    | 'MONEYSHYFT_DONOR_INTAKE_VALIDATION_FAILED'
    | 'MONEYSHYFT_DONOR_INTAKE_REFUSED_CAPACITY';
  message: string;
  data: {
    requestId?: string;
...
export type DonorRequestDetailResult = {
  ok: boolean;
  code:
    | 'MONEYSHYFT_DONOR_INTAKE_COMMITMENT_LINKED'
    | 'MONEYSHYFT_DONOR_INTAKE_REQUEST_RESOLVED'
    | 'MONEYSHYFT_DONOR_INTAKE_SCOPE_MISMATCH'
    | 'MONEYSHYFT_DONOR_INTAKE_REQUEST_NOT_FOUND'
    | 'MONEYSHYFT_DONOR_INTAKE_REQUEST_ID_REQUIRED';
  message: string;
  data: {
    requestId?: string;
    status?: string;
...
const SERVICEABLE_ZIPS_BY_TENANT_AND_ORG_UNIT: Record<string, Record<string, string[]>> = {
  'tenant-moneyshyft-alpha': {
    'org-moneyshyft-alpha-ops': ['46802', '46803', '46805', '46807'],
  },
  'tenant-moneyshyft-bravo': {
    'org-moneyshyft-bravo-ops': ['46774', '46725'],
  },
};
...
export class DonorIntakeService {
  ...
  submitDonorRequest(
    ...
    if (!validation.ok) {
      return {
        ok: false,
        code: 'MONEYSHYFT_DONOR_INTAKE_VALIDATION_FAILED',
  ...
  getDonorRequestDetail(
    ...
    if (normalizedRequestId.length === 0) {
      return {
        ok: false,
        code: 'MONEYSHYFT_DONOR_INTAKE_REQUEST_ID_REQUIRED',
    ...
    if (!request) {
      return {
        ok: false,
        code: 'MONEYSHYFT_DONOR_INTAKE_REQUEST_NOT_FOUND',
    ...
    if (request.tenantId !== context.tenantId || (context.orgUnitId && request.orgUnitId !== context.orgUnitId)) {
      return {
        ok: false,
        code: 'MONEYSHYFT_DONOR_INTAKE_SCOPE_MISMATCH',
    ...
    return {
      ok: true,
      code: 'MONEYSHYFT_DONOR_INTAKE_REQUEST_RESOLVED',
