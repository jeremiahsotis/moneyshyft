...
watch(() => props.category, (newCategory) => {
  if (newCategory) {
    categoryName.value = newCategory.category_name;
  }
}, { immediate: true });

async function handleSubmit() {
  if (!isValid.value || isSubmitting.value || !props.category) return;

  isSubmitting.value = true;
  try {
    await categoriesStore.updateCategory(props.category.category_id, {
