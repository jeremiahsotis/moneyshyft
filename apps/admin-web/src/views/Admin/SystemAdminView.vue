...
const loadTenants = async (): Promise<void> => {
  ...
  try {
    const response = await listTenants({
      tenantId: actorTenantId.value || undefined,
...
watch(activeSection, (section) => {
  ...
  if (section === 'audit') {
    if (!auditFilters.tenantId) {
      auditFilters.tenantId = selectedTenantId.value || actorTenantId.value || '';
    }
...
onMounted(async () => {
  await accessStore.refresh({ tenantId: actorTenantId.value || undefined });
