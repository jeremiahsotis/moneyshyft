...
const enforceDeliveryInsertionPolicy = (
  ...
  return refusal(
    'MONEYSHYFT_DELIVERY_INSERTION_CONSTRAINT',
...
export const evaluateSharedIntakePolicy = (
  ...
  if (!orgUnitId) {
    return refusal(
      'MONEYSHYFT_ORG_UNIT_ID_REQUIRED',
  ...
  if (!requestedAt || !requestedWindowStart || !requestedWindowEnd) {
    return refusal(
      'MONEYSHYFT_INTAKE_UTC_REQUIRED',
  ...
  if (requestedWindowEnd.getTime() <= requestedWindowStart.getTime()) {
    return refusal(
      'MONEYSHYFT_INTAKE_WINDOW_INVALID',
  ...
  if (payload.forceRefusal) {
    return refusal(
      'MONEYSHYFT_CAPACITY_UNAVAILABLE',
