...
const buildApp = ({
  ...
  if (injectUser) {
    app.use((req, _res, next) => {
      const tenantId = req.header('x-tenant-id') || 'tenant-moneyshyft-alpha';
      const orgUnitId = req.header('x-org-unit-id') || 'org-moneyshyft-alpha-ops';

      req.user = {
        userId: 'user-moneyshyft-dispatcher',
...
const buildHappyPayload = () => ({
  ...
  orgUnitId: 'org-moneyshyft-alpha-ops',
...
describe('MoneyShyft donor intake lifecycle routes', () => {
