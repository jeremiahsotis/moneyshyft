...
router.post('/users/password-reset', async (req: Request, res: Response) => {
  const {
    tenantId,
    orgUnitId,
    userId,
  ...
  const hasUserId = typeof userId === 'string' && userId.trim().length > 0;
  const hasUserEmail = typeof userEmail === 'string' && userEmail.trim().length > 0;
  const normalizedUserEmail = hasUserEmail ? userEmail.trim().toLowerCase() : '';
  const normalizedMode = typeof mode === 'string' ? mode.trim().toLowerCase() : '';
  const resolvedTenantId = typeof tenantId === 'string' && tenantId.trim().length > 0
    ? tenantId.trim()
    : resolveActiveTenantId(req);

  if (
    !resolvedTenantId
    || !isUuid(resolvedTenantId)
    || (orgUnitId !== undefined && (!isUuid(orgUnitId)))
    || (!hasUserId && !hasUserEmail)
    || (hasUserId && !isUuid(userId))
    || (hasUserEmail && !isEmail(normalizedUserEmail))
    || (normalizedMode !== 'temporary' && normalizedMode !== 'link')
    || typeof reason !== 'string'
    || reason.trim() === ''
  ) {
    return refusal(res, {
      code: 'PASSWORD_RESET_INPUT_INVALID',
      message: 'tenantId, userId(UUID) or userEmail, mode(link|temporary), and reason are required',
      refusalType: 'client',
      httpStatus: 400,
    });
  }
  ...
  try {
    ...
    const capabilityContext = await evaluateRequestCapabilities(
      db,
      actor,
      resolvedTenantId,
      typeof orgUnitId === 'string' ? orgUnitId.trim() : undefined,
    );

    if (!hasCapability(capabilityContext.roles, CAPABILITIES.USER_PASSWORD_RESET)) {
      return refusal(res, {
    ...
    const ensuredUser = await ensureScopedAdminUser(db, actor, {
      tenantId: resolvedTenantId,
      userId: hasUserId ? userId.trim() : undefined,
      userEmail: hasUserEmail ? normalizedUserEmail : undefined,
      allowInlineCreate: false,
      reason,
    });

    if (typeof orgUnitId === 'string' && orgUnitId.trim().length > 0) {
      const scopedOrgUnitId = orgUnitId.trim();
      const scopedMembership = await db
        .withSchema('platform')
        .table('org_unit_memberships')
        .where({ org_unit_id: scopedOrgUnitId, user_id: ensuredUser.userId })
        .first(['org_unit_id']);

      if (!scopedMembership) {
        return refusal(res, {
          code: 'PASSWORD_RESET_USER_OUT_OF_SCOPE',
          message: 'User was not found in active tenant scope',
          refusalType: 'client',
          httpStatus: 404,
        });
    ...
    if (normalizedMode === 'temporary') {
      if (typeof temporaryPassword !== 'string' || temporaryPassword.trim().length < 8) {
        return refusal(res, {
          code: 'PASSWORD_RESET_TEMPORARY_PASSWORD_INVALID',
          message: 'temporaryPassword must be at least 8 characters when mode=temporary',
          refusalType: 'client',
          httpStatus: 400,
        });
      }

      await PasswordResetService.applyTemporaryPassword(ensuredUser.userId, temporaryPassword);
      return success(res, {
        code: 'PASSWORD_RESET_TEMPORARY_PASSWORD_SET',
    ...
    const resetLink = await PasswordResetService.issueResetLink({
      userId: ensuredUser.userId,
      requestedByUserId: req.user?.userId || null,
      purpose: 'admin',
      resetBaseUrl: typeof resetBaseUrl === 'string' ? resetBaseUrl : undefined,
    });
    ...
    return success(res, {
      ...
      data: {
        ...
        expiresAtUtc: resetLink.expiresAtUtc,
      },
    });
  } catch (error) {
    if (handleScopeErrors(res, error) || handleUserResolutionError(res, error)) {
    ...
    if (handleForbiddenError(res, error, 'Insufficient permissions for password reset')) {
      return;
    }

    return errorEnvelope(res, {
      code: 'PASSWORD_RESET_FAILED',
      message: getMessage(error, 'Failed to reset password'),
      httpStatus: 500,
    });
  }
});
