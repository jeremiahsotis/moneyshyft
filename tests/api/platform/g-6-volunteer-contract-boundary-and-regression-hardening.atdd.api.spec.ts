  ...
  test.skip(
    ...
    async ({
      ...
      const refusalResponse = await apiRequest(request, {
        method: 'POST',
        path: `${storyG6Context.paths.threads}/${storyG6Context.threadIds.unclaimed}/messages`,
        headers: storyG6VolunteerHeaders,
        data: {
      ...
      expect(successBody.data?.feedback?.taxonomy ?? successBody.data?.uiFeedback?.severity).toBeTruthy();
      expect(refusalBody.refusalType).toBe('business');
      expect(refusalBody.data?.feedback?.taxonomy ?? refusalBody.data?.uiFeedback?.severity).toBeTruthy();
      expect(errorBody.data?.feedback?.taxonomy ?? errorBody.data?.uiFeedback?.severity).toBeTruthy();
