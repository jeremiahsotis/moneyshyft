...
test.describe('Story ux-r1 Mobile-First Inbox/Mine/Thread Redesign (ATDD E2E RED)', () => {
  test.skip(
    '[P0] persistent bottom navigation keeps Inbox Mine More visible without hidden fourth primary tab during surface transitions @P0',
    async ({ page }) => {
  ...
  test.skip(
    '[P1] responsive mobile tablet and desktop breakpoints preserve context voicemail indicators and action discoverability without hidden policy paths @P1',
    async ({ page }) => {
      const context = createStoryUxR1Context();
      await login(page);
