...
test.describe('Story 2-2 Donor Self-Service Pickup Intake with Capacity Check (Automate E2E Expansion)', () => {
  test.skip('[P1] keeps submit disabled until donor intake required fields are complete @P1', async ({ page, story22Context }) => {
    ...
    await expect(page.getByTestId('routeshyft-donor-intake-submit')).toBeDisabled();
    await page.getByTestId('routeshyft-donor-intake-item-summary').fill('Sofa and lamp');
    await page.getByTestId('routeshyft-donor-intake-window-start').fill('2026-02-27T14:00');
    await page.getByTestId('routeshyft-donor-intake-window-end').fill('2026-02-27T16:00');
    await expect(page.getByTestId('routeshyft-donor-intake-submit')).toBeEnabled();
  });
  ...
  test.skip('[P1] renders schedulable slot outcomes with next-step guidance and no refusal banner @P1', async ({ page, story22Context }) => {
    ...
    await page.goto(story22Context.paths.ui + '?tenantId=' + story22Context.tenantId + '&orgUnitId=' + story22Context.orgUnitId);
    await page.getByTestId('routeshyft-donor-intake-submit').click();
    ...
    await expect(page.getByTestId('routeshyft-donor-intake-refusal-banner')).toBeHidden();
  });
  ...
  test.skip('[P2] prevents duplicate submissions when donor double-clicks submit during request in-flight @P2', async ({ page, story22Context }) => {
    ...
    await page.goto(story22Context.paths.ui + '?tenantId=' + story22Context.tenantId + '&orgUnitId=' + story22Context.orgUnitId);
    const submitButton = page.getByTestId('routeshyft-donor-intake-submit');
