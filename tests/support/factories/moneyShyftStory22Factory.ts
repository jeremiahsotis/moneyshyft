...
export function createStory22Context(overrides: Story22ContextOverrides = {}): Story22Context {
  return {
    ...
    userId: overrides.userId ?? 'user-moneyshyft-22-' + randomUUID().slice(0, 8),
    correlationId: overrides.correlationId ?? 'corr-moneyshyft-22-' + randomUUID().slice(0, 8),
    csrfToken: overrides.csrfToken ?? 'csrf-moneyshyft-22-' + randomUUID().slice(0, 8),
    ...
    successCode: 'MONEYSHYFT_DONOR_INTAKE_SLOTS_AVAILABLE',
    refusalCode: 'MONEYSHYFT_DONOR_INTAKE_REFUSED_CAPACITY',
    linkageCode: 'MONEYSHYFT_DONOR_INTAKE_COMMITMENT_LINKED',
    statusLabel: 'Schedulable',
    requiredTestIds: [
      'moneyshyft-donor-intake-submit',
      'moneyshyft-donor-intake-slot-list',
      'moneyshyft-donor-intake-refusal-banner',
      'moneyshyft-donor-intake-refusal-code',
      'moneyshyft-donor-intake-next-steps',
      'moneyshyft-donor-intake-alternatives',
    ],
  };
}
