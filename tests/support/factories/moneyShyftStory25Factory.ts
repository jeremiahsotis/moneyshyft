...
export function createStory25Context(overrides: Story25ContextOverrides = {}): Story25Context {
  return {
    ...
    userId: overrides.userId ?? 'user-moneyshyft-25-' + randomUUID().slice(0, 8),
    correlationId: overrides.correlationId ?? 'corr-moneyshyft-25-' + randomUUID().slice(0, 8),
    csrfToken: overrides.csrfToken ?? 'csrf-moneyshyft-25-' + randomUUID().slice(0, 8),
    ...
    successCode: 'MONEYSHYFT_REFUSAL_RECORDED',
    refusalCode: 'MONEYSHYFT_REFUSAL_REASON_INVALID',
    linkageCode: 'MONEYSHYFT_REFUSAL_HISTORY_UPDATED',
    statusLabel: 'Refused',
    requiredTestIds: [
      'moneyshyft-refusal-submit',
      'moneyshyft-refusal-outcome-banner',
      'moneyshyft-refusal-code',
      'moneyshyft-refusal-alternatives-list',
      'moneyshyft-refusal-next-steps',
      'moneyshyft-refusal-audit-history',
    ],
  };
}
