...
export function createStory23Context(overrides: Story23ContextOverrides = {}): Story23Context {
  return {
    ...
    userId: overrides.userId ?? 'user-routeshyft-23-' + randomUUID().slice(0, 8),
    correlationId: overrides.correlationId ?? 'corr-routeshyft-23-' + randomUUID().slice(0, 8),
    csrfToken: overrides.csrfToken ?? 'csrf-routeshyft-23-' + randomUUID().slice(0, 8),
    ...
    successCode: 'ROUTESHYFT_CASHIER_INTAKE_ACCEPTED',
    refusalCode: 'ROUTESHYFT_CASHIER_INTAKE_REFUSED',
    linkageCode: 'ROUTESHYFT_CASHIER_INTAKE_COMMITMENT_LINKED',
