...
import {
  ...
  type Story24Context,
  type Story24Payload,
} from '../factories/moneyShyftStory24Factory';
...
async function loginStory24Scope(
  ...
  const treeResponse = await apiRequest(request, {
    ...
    headers: {
      cookie: authCookie,
      'x-correlation-id': 'corr-moneyshyft-24-tree-' + randomUUID().slice(0, 8),
    },
  });
  ...
  if (!orgUnitId) {
    ...
    const bootstrapResponse = await apiRequest(request, {
      ...
      headers: {
        ...
        'x-correlation-id': 'corr-moneyshyft-24-node-' + randomUUID().slice(0, 8),
    ...
    if (!orgUnitId) {
      const finalTreeResponse = await apiRequest(request, {
        ...
        headers: {
          cookie: authCookie,
          'x-correlation-id': 'corr-moneyshyft-24-tree-retry-' + randomUUID().slice(0, 8),
        },
      });
...
export const test = base.extend<Story24Fixtures>({
  story24Context: async ({ request }, use) => {
    ...
    await use(createStory24Context({
      ...
      userId: scope.userId,
      correlationId: 'corr-moneyshyft-24-' + randomUUID().slice(0, 8),
      csrfToken: 'csrf-moneyshyft-24-' + randomUUID().slice(0, 8),
    }));
  },
